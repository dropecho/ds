...
class dropecho_ds_GraphNode {
	...
	neighborIds() {
		return this.outNeighborIds().concat(this.inNeighborIds());
	}
	neighbors(filter) {
		return this.outNeighbors().concat(this.inNeighbors());
	}
	edgeData(toId) {
...
class dropecho_ds_Graph {
	...
	remove(id) {
		let _this = this.edges;
		if(Object.prototype.hasOwnProperty.call(_this.h,id)) {
			delete(_this.h[id]);
		}
		let _this1 = this.nodes;
	...
	inNeighborIds(node,filter) {
		let _g = [];
		let _g1 = haxe_ds_StringMap.kvIterator(this.edges.h);
		while(_g1.hasNext()) {
			let _g2 = _g1.next();
			let id = _g2.key;
			let edge = _g2.value;
	...
	outNeighborIds(node,filter) {
		...
		let _g = [];
		let _g1 = haxe_ds_StringMap.kvIterator(this.edges.h[node.id].h);
		while(_g1.hasNext()) {
			let _g2 = _g1.next();
			let id = _g2.key;
	...
	toString() {
		...
		while(node.hasNext()) {
			...
			adjList += node1.id;
			adjList += "\t-> ";
			let neighbors = this.outNeighbors(node1);
			let _g = 0;
		...
		let node1 = haxe_ds_StringMap.valueIterator(this.nodes.h);
		while(node1.hasNext()) {
			let node = node1.next();
			adjList += node.id;
			adjList += "\t-> ";
...
class dropecho_ds_graph_Grammar {
	static replace(node,newNode) {
	}
}
$hx_exports["graph"]["grammar"] = dropecho_ds_graph_Grammar;
dropecho_ds_graph_Grammar.__name__ = true;
